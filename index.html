<!DOCTYPE html>
<html lang="en" dir="ltr">
<head>

    <!-- Required meta tags -->
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no, viewport-fit=cover" />
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <meta name="description" content="" />
    <meta name="author" content="" />      

    <!-- Title -->
    <title>Royce - Hotel and Casino</title>

    <!-- All Styles -->
    <link rel="stylesheet" href="src/assets/css/main.min.css">

    <!-- Favicons -->
    <link rel="apple-touch-icon" sizes="180x180" href="src/assets/img/favicon/apple-touch-icon.png" />
    <link rel="icon" type="image/png" sizes="256x256"  href="src/assets/img/favicon/android-chrome-256x256.png">
    <link rel="icon" type="image/png" sizes="192x192"  href="src/assets/img/favicon/android-chrome-192x192.png">    
    <link rel="icon" type="image/png" sizes="32x32" href="src/assets/img/favicon/favicon-32x32.png" />
    <link rel="icon" type="image/png" sizes="16x16" href="src/assets/img/favicon/favicon-16x16.png" />
    <link rel="icon" type="image/png" href="src/assets/img/favicon/favicon.ico" />
    <link rel="manifest" href="src/assets/img/favicon/site.webmanifest" />
    <link rel="mask-icon" href="src/assets/img/favicon/safari-pinned-tab.svg" color="#5bbad5" />
    <meta name="msapplication-TileColor" content="#990100" />
    <meta name="theme-color" content="#ffffff" />       
    
</head>
<body class="d-flex flex-column justify-content-between">

<!-- =========================
    Pre-loader 
============================== -->

<!-- Start .loading -->
<div id="loading">
    <div class="loading__center">
        <div class="loading__position">
            <div id="object"></div>
        </div>
    </div>
</div>
<!-- End /.loading -->
              

<!-- =========================
    Icons 
============================== -->

<svg aria-hidden="true" style="position: absolute; width: 0; height: 0; overflow: hidden;" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    <defs>

        <!-- Start Close Icon -->
        <symbol id="icon-close" viewBox="0 0 32 32">
            <title>close</title>
            <path d="M18.314 16l13.686 13.686-2.315 2.314-13.685-13.686-13.686 13.686-2.314-2.314 13.685-13.686-13.685-13.686 2.314-2.314 13.686 13.686 13.685-13.686 2.315 2.314z"></path>
        </symbol><!-- End Close Icon -->

        <!-- Start Pin Sharp Circle Icon -->
        <symbol id="icon-pin_sharp_circle" viewBox="0 0 22 32">
            <title>pin_sharp_circle</title>
            <path d="M14.4 11.55c0 1.768-1.432 3.2-3.2 3.2s-3.2-1.432-3.2-3.2c0-1.768 1.432-3.2 3.2-3.2s3.2 1.432 3.2 3.2zM11.2 27.2c0 0-8-11.2-8-16 0-4.411 3.589-8 8-8s8 3.589 8 8c0 4.8-8 16-8 16zM11.2 0c-6.186 0-11.2 5.014-11.2 11.2s11.2 20.8 11.2 20.8c0 0 11.2-14.614 11.2-20.8s-5.014-11.2-11.2-11.2z"></path>
        </symbol><!-- End Pin Sharp Circle Icon -->
        
        <!-- Start Checked Icon -->
        <symbol id="icon-checked" viewBox="0 0 32 32">
            <title>checked</title>
            <path d="M16 28.8c-7.069 0-12.8-5.731-12.8-12.8v0c0-7.069 5.731-12.8 12.8-12.8v0c7.069 0 12.8 5.731 12.8 12.8v0c0 7.069-5.731 12.8-12.8 12.8v0zM16 32v0c8.837 0 16-7.163 16-16v0c0-8.837-7.163-16-16-16v0c-8.837 0-16 7.163-16 16v0c0 8.837 7.163 16 16 16zM14.523 20.8l7.877-7.68-1.969-1.92-5.908 5.761-2.955-2.881-1.969 1.92 4.923 4.801z"></path>
        </symbol><!-- End Checked Icon -->

    </defs>
</svg>

<!-- =========================
    1. Section - Header 
============================== -->

<!-- Start .Navbar -->
<header class="Navbar sticky-header">   

    <!-- Start .Navbar__nav -->
    <nav class="Navbar__nav">

        <!-- Start .container -->
        <div class="container d-flex justify-content-between align-items-center">

            <!-- Start .Navbar-logo -->
<div class="Navbar-logo">

    <!-- Start .Navbar-logo__image -->
    <h1 class="Navbar-logo__image">
        <a href="/" title="Rafoz Farzo - Photography Template" class="Navbar-logo__link"></a>
    </h1><!-- End /.Navbar-logo__image-->

    <!-- Start .Navbar-logo__info -->
    <div class="Navbar-logo__info label label--gray">
        <div><span class="semibold">Казино Отель</span> для</div> шведских людей 
    </div><!-- End /.Navbar-logo__info -->

</div><!-- End /.Navbar-logo -->

            <!-- =========================
                Burger Right Menu Icon
            ============================== -->                

            <!-- Start .burger-icon -->
            <div class="cd-nav-trigger burger-icon burger-icon--right">
                <div class="cd-nav__divider"></div>
                <div class="cd-nav__divider"></div> 
                <div class="cd-nav__divider"></div>
            </div><!-- End /.burger-icon -->     

            <!-- Start .Menu -->
<ul class="Menu Menu--line">

    <!-- Start .Menu-item -->
    <li class="Menu-item">
        <a href="/" title="Главная" class="Menu-item__link">
        Главная
        </a>                
    </li><!-- End /.Menu-item-->  

    <!-- Start .Menu-item -->
    <li class="Menu-item">
        <a href="#" title="Лучшие отели казино" class="Menu-item__link">
        Лучшие отели казино 
        </a>
    </li><!-- End /.Menu-item-->    

    <!-- Start .Menu-item -->
    <!-- <li class="Menu-item">
        <a href="/about.html" title="Что мы делаем?" class="Menu-item__link">
        Что мы делаем?
        </a>
    </li> -->
    <!-- End /.Menu-item--> 

    <!-- Start .Menu-item -->
    <li class="Menu-item">
        <a href="/contact.html" title="Контакты" class="Menu-item__link">
        Контакты
        </a>
    </li><!-- End /.Menu-item-->    

</ul><!-- End /.Menu-->   

        </u><!-- End /.container-->

    </nav><!-- End /.Navbar__nav-->

</header><!-- End /.Navbar-->


<main class="sticky">
    <article>
        
        <!-- =========================
    2. Section - Banner 
============================== -->

<!-- Start .Banner -->
<section class="Banner section clearfix">
    <div class="container">
        <div class="row">
            <div class="col-md-12 col-lg-6">

                <!-- Start .Banner__info -->
<div class="Banner__info">

    <!-- Start .Banner__title -->
    <h2 class="Banner__title">
        Лучший скандинавский или европейский <span class="Banner__label">отель казино</span> для шведских людей
    </h2><!-- End /.Banner__title -->

    <!-- Start .Banner__description -->
    <p class="Banner__description">
        Здесь вы можете найти отзывы экспертов о лучших отелях казино в <span class="semibold">Европе</span> для вас. Выберите <span class="semibold">отели казино</span>, которые подходят вам лучше всего в списке.
    </p><!-- End /.Banner__description -->
    
</div><!-- Start /.Banner__info -->

            </div>
            <div class="col-md-12 col-lg-4 offset-lg-2">

                <!-- Start .Banner-card -->
<div class="Banner-card card card--padd card--bg">
                    
    <!-- Start .Banner-card__header -->
    <div class="Banner-card__header">
        <img src="src/assets/img/icons/locked.svg" alt="locked" title="locked" />
        <div class="Banner-card__label">
            <h3 class="Banner-card__title">
                Все отели <div class="semibold">100% гарантированы</div>
            </h3>
        </div>
    </div><!-- End /.Banner-card__header -->

    <!-- Start .Banner-card-list -->
    <div class="Banner-card-list">

        <!-- Start .Banner-card-list__item -->
        <div class="Banner-card-list__item">
            <svg class="icon icon-checked">
                <use xlink:href="#icon-checked"></use>
            </svg>
            <h3 class="Banner-card-list__name">Лучшие отели</h3>
        </div><!-- End /.Banner-card-list__item -->

        <!-- Start .Banner-card-list__item -->
        <div class="Banner-card-list__item">
            <svg class="icon icon-checked">
                <use xlink:href="#icon-checked"></use>
            </svg>
            <h3 class="Banner-card-list__name">Поддержка и общение лучшие в отелях.</h3>
        </div><!-- End /.Banner-card-list__item -->

        <!-- Start .Banner-card-list__item -->
        <div class="Banner-card-list__item">
            <svg class="icon icon-checked">
                <use xlink:href="#icon-checked"></use>
            </svg>
            <h3 class="Banner-card-list__name">Прямая ссылка</h3>
        </div><!-- End /.Banner-card-list__item -->

    </div><!-- End /.Banner-card-list -->

</div><!-- End /.Banner-card -->
                
            </div>
        </div>
    </div>
</section><!-- End /.Banner -->

        <!-- =========================
    3. Section - Hotels 
============================== -->

<!-- Start .Hotels -->
<section class="Hotels section clearfix">
    <div class="container">

        <!-- Start .Hotels-table -->
        <div class="Hotels-table">

            <!-- Start .Hotels-table__header -->
            <div class="Hotels-table__header">
                    <div class="row">

                        <div class="col-lg-7">
                            <div class="row"> 
                                <div class="col-lg-6">
                                    
                                    <!-- Start .Hotels-table__head -->
                                    <div class="Hotels-table__head">
                                        <span class="semibold Hotels-table__id">#1</span>
                                        <strong class="Hotels-table__title">ГОСТИНИЦА</strong>
                                    </div><!-- End / .Hotels-table__head -->

                                </div>
                                <div class="col-lg-6">
                                    
                                    <!-- Start .Hotels-table__head -->
                                    <div class="Hotels-table__head">
                                        <span class="semibold Hotels-table__id">#2</span>
                                        <strong class="Hotels-table__title">АДРЕС</strong>
                                    </div><!-- End / .Hotels-table__head -->
                                    
                                </div>                                                                        
                            </div>
                        </div>

                        <div class="col-lg-5">
                            <div class="row">
                                <div class="col-lg-5">
                                    
                                    <!-- Start .Hotels-table__head -->
                                    <div class="Hotels-table__head">
                                        <span class="semibold Hotels-table__id">#3</span>
                                        <strong class="Hotels-table__title">ПОЛОЖЕНИЕ</strong>
                                    </div><!-- End / .Hotels-table__head -->
                        
                                </div>
                                <div class="col-lg-7">
                                    
                                    <!-- Start .Hotels-table__head -->
                                    <div class="Hotels-table__head">
                                        <span class="semibold Hotels-table__id">#4</span>
                                        <strong class="Hotels-table__title">ПОДРОБНЫЙ ОБЗОР</strong>
                                    </div><!-- End / .Hotels-table__head -->
                    
                                </div>                                                                        
                            </div>
                        </div>

                    </div>
            </div><!-- End /.Hotels-table__header -->

            <!-- Start .Hotels-table__body -->
            <div class="Hotels-table__body">

                <!-- Start .Hotels-table__row -->
                <div class="Hotels-table__row">
                    <div class="row">

                        <div class="col-lg-7">
                            <div class="row">
                                <div class="col-md-6 col-lg-6">
                                    <div class="Hotels-table__sort">
                                        <img src="src/assets/img/icons/number-one.svg" alt="number-one" title="number-one" />
                                        <img src="src/assets/img/hotels/hotel1.png" alt="hotel1" title="hotel1" class="Hotel__image" />
                                    </div>
                                </div>
                                <div class="col-md-6 col-lg-6 valign">
                                    <p class="Hotels__description">3 Avenue Georges Bassinet, 69260 Charbonnières-les-Bains, Frankrike</p>
                                </div>                                                                        
                            </div>
                        </div>

                        <div class="col-lg-5">
                            <div class="row">
                                <div class="col-md-6 col-lg-5">

                                    <!-- Start .Hotels__rating -->
<div class="Hotels__rating">

    <span class="semibold Hotels__score">9.8</span>
    <span class="Hotels__status">Экспертный рейтинг</span>

    
<!-- Start .stars -->
<div class="stars">
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
</div><!-- End /.stars -->

</div><!-- End /.Hotels__rating -->

                                </div>
                                <div class="col-md-6 col-lg-7 valign d-flex justify-content-end">
                                    <a href="hotel.html" class="button" title="более">ПОСЕТИТЕ СТРАНИЦА</a>
                                </div>                                                                        
                            </div>
                        </div>

                    </div>
                </div><!-- End /.Hotels-table__row -->

                <!-- Start .Hotels-table__row -->
                <div class="Hotels-table__row">
                    <div class="row">

                        <div class="col-lg-7">
                            <div class="row">
                                <div class="col-md-6 col-lg-6">
                                    <div class="Hotels-table__sort">
                                        <span class="Hotels-table__number">2</span>
                                        <img src="src/assets/img/hotels/hotel2.png" alt="hotel1" title="hotel1" class="Hotel__image" />                                        
                                    </div>
                                </div>
                                <div class="col-md-6 col-lg-6 valign">
                                    <p class="Hotels__description">150 Piccadilly, St James, London W1J 9BR, Storbritannien</p>
                                </div>                                                                        
                            </div>
                        </div>

                        <div class="col-lg-5">
                            <div class="row">
                                <div class="col-md-6 col-lg-5">

                                    <!-- Start .Hotels__rating -->
<div class="Hotels__rating">

    <span class="semibold Hotels__score">9.8</span>
    <span class="Hotels__status">Экспертный рейтинг</span>

    
<!-- Start .stars -->
<div class="stars">
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
</div><!-- End /.stars -->

</div><!-- End /.Hotels__rating -->

                                </div>
                                <div class="col-md-6 col-lg-7 valign d-flex justify-content-end">
                                    <a href="hotel.html" class="button" title="более">ПОСЕТИТЕ СТРАНИЦА</a>
                                </div>                                                                        
                            </div>
                        </div>

                    </div>
                </div><!-- End /.Hotels-table__row -->

                <!-- Start .Hotels-table__row -->
                <div class="Hotels-table__row">
                    <div class="row">

                        <div class="col-lg-7">
                            <div class="row">
                                <div class="col-md-6 col-lg-6">
                                    <div class="Hotels-table__sort">
                                        <span class="Hotels-table__number">3</span>
                                        <img src="src/assets/img/hotels/hotel3.png" alt="hotel1" title="hotel1" class="Hotel__image" />                                        
                                    </div>
                                </div>
                                <div class="col-md-6 col-lg-6 valign">
                                    <p class="Hotels__description">U Obecního domu 660/3, 110 00 Staré Město, Tjeckien</p>
                                </div>                                                                        
                            </div>
                        </div>

                        <div class="col-lg-5">
                            <div class="row">
                                <div class="col-md-6 col-lg-5">

                                    <!-- Start .Hotels__rating -->
<div class="Hotels__rating">

    <span class="semibold Hotels__score">9.8</span>
    <span class="Hotels__status">Экспертный рейтинг</span>

    
<!-- Start .stars -->
<div class="stars">
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
</div><!-- End /.stars -->

</div><!-- End /.Hotels__rating -->

                                </div>
                                <div class="col-md-6 col-lg-7 valign d-flex justify-content-end">
                                    <a href="hotel.html" class="button" title="более">ПОСЕТИТЕ СТРАНИЦА</a>
                                </div>                                                                        
                            </div>
                        </div>

                    </div>
                </div><!-- End /.Hotels-table__row -->

                <!-- Start .Hotels-table__row -->
                <div class="Hotels-table__row">
                    <div class="row">

                        <div class="col-lg-7">
                            <div class="row">
                                <div class="col-md-6 col-lg-6">
                                    <div class="Hotels-table__sort">
                                        <span class="Hotels-table__number">4</span>
                                        <img src="src/assets/img/hotels/hotel4.png" alt="hotel1" title="hotel1" class="Hotel__image" />                                        
                                    </div>
                                </div>
                                <div class="col-md-6 col-lg-6 valign">
                                    <p class="Hotels__description">Adolfo Diaz Ambrona, 11, 06006 Badajoz, Spain</p>
                                </div>                                                                        
                            </div>
                        </div>

                        <div class="col-lg-5">
                            <div class="row">
                                <div class="col-md-6 col-lg-5">

                                    <!-- Start .Hotels__rating -->
<div class="Hotels__rating">

    <span class="semibold Hotels__score">9.8</span>
    <span class="Hotels__status">Экспертный рейтинг</span>

    
<!-- Start .stars -->
<div class="stars">
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
</div><!-- End /.stars -->

</div><!-- End /.Hotels__rating -->

                                </div>
                                <div class="col-md-6 col-lg-7 valign d-flex justify-content-end">
                                    <a href="hotel.html" class="button" title="более">Go to Hotel Site</a>   
                                </div>                                                                        
                            </div>
                        </div>

                    </div>
                </div><!-- End /.Hotels-table__row -->

                <!-- Start .Hotels-table__row -->
                <div class="Hotels-table__row">
                    <div class="row">

                        <div class="col-lg-7">
                            <div class="row">
                                <div class="col-md-6 col-lg-6">
                                    <div class="Hotels-table__sort">
                                        <span class="Hotels-table__number">5</span>
                                        <img src="src/assets/img/hotels/hotel5.png" alt="hotel1" title="hotel1" class="Hotel__image" />                                        
                                    </div>
                                </div>
                                <div class="col-md-6 col-lg-6 valign">
                                    <p class="Hotels__description">Adolfo Diaz Ambrona, 11, 06006 Badajoz, Spain</p>
                                </div>                                                                        
                            </div>
                        </div>

                        <div class="col-lg-5">
                            <div class="row">
                                <div class="col-md-6 col-lg-5">

                                    <!-- Start .Hotels__rating -->
<div class="Hotels__rating">

    <span class="semibold Hotels__score">9.8</span>
    <span class="Hotels__status">Экспертный рейтинг</span>

    
<!-- Start .stars -->
<div class="stars">
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
    <img src="src/assets/img/icons/star.svg" alt="star" title="star" class="stars__item" />
</div><!-- End /.stars -->

</div><!-- End /.Hotels__rating -->

                                </div>
                                <div class="col-md-6 col-lg-7 valign d-flex justify-content-end">
                                    <a href="hotel.html" class="button" title="более">ПОСЕТИТЕ СТРАНИЦА</a>
                                </div>                                                                        
                            </div>
                        </div>

                    </div>
                </div><!-- End /.Hotels-table__row -->
                
            </div><!-- End /.Hotels-table__body -->

        </div><!-- End /.Hotels-table -->

    </div>
</section><!-- End /.Hotels -->

        <!-- =========================
    4. Section - Review 
============================== --> 

<!-- Start .Review -->
<section class="Review section clearfix">
    <div class="container">
        <div class="row">
            <div class="col-lg-5">
    
                <!-- Start .text -->
                <p class="text">
                    Если вы любите играть в казино во время путешествия со своим партнером, 
                    вам может быть сложно решить, какой отель на каботаре выбрать. 
                    Нет проблем, используя наш список, вам гарантированно найдется тот, который подходит. 
                    Там вы можете наслаждаться настольными играми, слотами, покером и играми с большим трудом. 
                    Вы также можете получить специальные бонусы, если это ваш первый визит, или стать членом лояльности, 
                    если вы вернетесь несколько раз. Взгляните на наш список 10 лучших отелей в Европе, чтобы узнать, 
                    кто, скорее всего, подойдет вам
                </p><!-- End /.text -->
    
            </div>
            <div class="col-lg-6 offset-lg-1">
    
                <!-- Start .testimonial -->
<div class="testimonial">

    <!-- Start .testimonial-slider -->
    <div class="testimonial-slider">

        <!-- Start .testimonial-slider__item -->
        <div class="testimonial-slider__item">

            <!-- Start .testimonial-slider__review -->
            <div class="testimonial-slider__review">
                <div class="testimonial-slider__quote">
                    <h3 class="testimonial-slider__text">Ребята смогли быстро погрузиться в специфику нашего бизнеса, изучить процессы, предложить улучшения и способы модернизации существующей инфраструктуры.</h3>
                    <img src="src/assets/img/icons/quotes.svg" alt="quotes" class="testimonial-slider__icon" />                                                           
                </div>
                <div class="testimonial-slider__caret"></div> 
            </div><!-- End /.testimonial-slider__review -->

            <!-- Start .testimonial-slider__details -->
            <div class="testimonial-slider__details">

                <!-- Start .testimonial-slider__user -->
                <div class="testimonial-slider__user">

                    <!-- Start .testimonial-slider-avatar -->
                    <div class="testimonial-slider-avatar">
                        <img src="src/assets/img/avatars/avatar.png" alt="Svea Dmitiros" title="Svea Dmitiros" class="testimonial-slider-avatar__image" />
                    </div><!-- End /.testimonial-slider-avatar -->

                    <h6 class="testimonial-slider__name semibold">Svea Dmitiros</h6>

                </div><!-- End /.testimonial-slider__user -->

            </div><!-- End /.testimonial-slider__details -->

        </div><!-- End /.testimonial-slider__item -->

        <!-- Start .testimonial-slider__item -->
        <div class="testimonial-slider__item">

            <!-- Start .testimonial-slider__review -->
            <div class="testimonial-slider__review">
                <div class="testimonial-slider__quote">
                    <h3 class="testimonial-slider__text">Доброго времени суток. Спасибо за прекрасную работу. Хотела бы сейчас прийти к Вам с бутылочкой шампанского.</h3>
                    <img src="src/assets/img/icons/quotes.svg" alt="quotes" class="testimonial-slider__icon" />                                                           
                </div>
                <div class="testimonial-slider__caret"></div> 
            </div><!-- End /.testimonial-slider__review -->

            <!-- Start .testimonial-slider__details -->
            <div class="testimonial-slider__details">

                <!-- Start .testimonial-slider__user -->
                <div class="testimonial-slider__user">

                    <!-- Start .testimonial-slider-avatar -->
                    <div class="testimonial-slider-avatar">
                        <img src="src/assets/img/avatars/avatar2.jpg" alt="Leo Gill" title="Leo Gill" class="testimonial-slider-avatar__image" />
                    </div><!-- End /.testimonial-slider-avatar -->

                    <h6 class="testimonial-slider__name semibold">Leo Gill</h6>

                </div><!-- End /.testimonial-slider__user -->

            </div><!-- End /.testimonial-slider__details -->

        </div><!-- End /.testimonial-slider__item -->

        <!-- Start .testimonial-slider__item -->
        <div class="testimonial-slider__item">

            <!-- Start .testimonial-slider__review -->
            <div class="testimonial-slider__review">
                <div class="testimonial-slider__quote">
                    <h3 class="testimonial-slider__text">Хочу сказать большое спасибо за отличный менеджмент и общее исполнение проекта. Сайт получился отличный, и с вами, как всегда, было очень приятно и комфортно работать! Спасибо!</h3>
                    <img src="src/assets/img/icons/quotes.svg" alt="quotes" class="testimonial-slider__icon" />                                                           
                </div>
                <div class="testimonial-slider__caret"></div> 
            </div><!-- End /.testimonial-slider__review -->

            <!-- Start .testimonial-slider__details -->
            <div class="testimonial-slider__details">

                <!-- Start .testimonial-slider__user -->
                <div class="testimonial-slider__user">

                    <!-- Start .testimonial-slider-avatar -->
                    <div class="testimonial-slider-avatar">
                        <img src="src/assets/img/avatars/avatar3.jpg" alt="Miyah Myles" title="Miyah Myles" class="testimonial-slider-avatar__image" />
                    </div><!-- End /.testimonial-slider-avatar -->

                    <h6 class="testimonial-slider__name semibold">Miyah Myles</h6>

                </div><!-- End /.testimonial-slider__user -->

            </div><!-- End /.testimonial-slider__details -->

        </div><!-- End /.testimonial-slider__item -->

        <!-- Start .testimonial-slider__item -->
        <div class="testimonial-slider__item">

            <!-- Start .testimonial-slider__review -->
            <div class="testimonial-slider__review">
                <div class="testimonial-slider__quote">
                    <h3 class="testimonial-slider__text">Работа с вами — одно удовольствие! Нам есть с чем сравнить, ведь от предыдущего подрядчика мы, в прямом смысле слова, сбежали!</h3>
                    <img src="src/assets/img/icons/quotes.svg" alt="quotes" class="testimonial-slider__icon" />                                                           
                </div>
                <div class="testimonial-slider__caret"></div> 
            </div><!-- End /.testimonial-slider__review -->

            <!-- Start .testimonial-slider__details -->
            <div class="testimonial-slider__details">

                <!-- Start .testimonial-slider__user -->
                <div class="testimonial-slider__user">

                    <!-- Start .testimonial-slider-avatar -->
                    <div class="testimonial-slider-avatar">
                        <img src="src/assets/img/avatars/avatar4.jpg" alt="Sasha Ho" title="Sasha Ho" class="testimonial-slider-avatar__image" />
                    </div><!-- End /.testimonial-slider-avatar -->

                    <h6 class="testimonial-slider__name semibold">Sasha Ho</h6>

                </div><!-- End /.testimonial-slider__user -->

            </div><!-- End /.testimonial-slider__details -->

        </div><!-- End /.testimonial-slider__item -->

    </div><!-- End /.testimonial-slider -->

</div><!-- End /.testimonial -->
    
            </div> 
        </div>       
    </div>
</section><!-- End /.Review -->


    </article>
</main>

<!-- =========================
    Mobile - Burger Right Menu  
============================== -->

<!-- Start .burger-menu -->
<nav class="cd-nav-container burger-menu burger-menu--right" id="cd-nav">
    <div class="burger-menu__open">

        <!-- Start .burger-menu__logo -->
        <a href="index.html" class="burger-menu__logo" title="Rafoz Farzo - Photography Template"></a><!-- End /.burger-menu__logo -->

        <!-- Start .cd-close-nav -->
        <div class="cd-nav-trigger cd-close-nav">
            <div class="cd-nav__divider cd-nav__divider--top"></div>
            <div class="cd-nav__divider cd-nav__divider--bottom"></div>
        </div><!-- End /.cd-close-nav -->

    </div>

    <!-- Start .burger-menu__list -->
    <ul class="burger-menu-list">

        <!-- Start .burger-menu-list__item -->
        <li class="burger-menu-list__item">
            <a href="/" title="Главная" class="burger-menu-list__link">Главная</a>
        </li><!-- End /.burger-menu-list__item -->

        <!-- Start .burger-menu-list__item -->
        <li class="burger-menu-list__item">
            <a href="#" title="Лучшие отели казино" class="burger-menu-list__link">Лучшие отели казино</a>
        </li><!-- End /.burger-menu-list__item -->

        <!-- Start .burger-menu-list__item -->
        <li class="burger-menu-list__item">
            <a href="/about.html" title="Что мы делаем?" class="burger-menu-list__link">Что мы делаем?</a>
        </li><!-- End /.burger-menu-list__item -->

        <!-- Start .burger-menu-list__item -->
        <li class="burger-menu-list__item">
            <a href="/contact.html" title="Контакты" class="burger-menu-list__link">Контакты</a>
        </li><!-- End /.burger-menu-list__item -->

    </ul><!-- End /.burger-menu__list -->
</nav><!-- End /.burger-menu -->

<!-- Start .cd-overlay --> 
<div class="cd-overlay"></div><!-- End /.cd-overlay --> 

<!-- Start .scroll-up -->
<div class="scroll-up">
    <div class="scroll-up__icon"></div>
</div><!-- End /.scroll-up -->
    

<!-- =========================
    5. Section - Footer 
============================== -->

<!-- Start .Navbar -->
<footer class="Navbar clearfix">   

    <!-- Start .Navbar__nav -->
    <nav class="Navbar__nav">

        <!-- Start .container -->
        <div class="container d-flex justify-content-between align-items-center">

            <!-- Start .Navbar-logo -->
<div class="Navbar-logo">

    <!-- Start .Navbar-logo__image -->
    <h1 class="Navbar-logo__image">
        <a href="/" title="Rafoz Farzo - Photography Template" class="Navbar-logo__link"></a>
    </h1><!-- End /.Navbar-logo__image-->

    <!-- Start .Navbar-logo__info -->
    <div class="Navbar-logo__info label label--gray">
        <div><span class="semibold">Казино Отель</span> для</div> шведских людей 
    </div><!-- End /.Navbar-logo__info -->

</div><!-- End /.Navbar-logo -->   

            <!-- Start .Menu -->
<ul class="Menu Menu--line">

    <!-- Start .Menu-item -->
    <li class="Menu-item">
        <a href="/" title="Главная" class="Menu-item__link">
        Главная
        </a>                
    </li><!-- End /.Menu-item-->  

    <!-- Start .Menu-item -->
    <li class="Menu-item">
        <a href="#" title="Лучшие отели казино" class="Menu-item__link">
        Лучшие отели казино 
        </a>
    </li><!-- End /.Menu-item-->    

    <!-- Start .Menu-item -->
    <!-- <li class="Menu-item">
        <a href="/about.html" title="Что мы делаем?" class="Menu-item__link">
        Что мы делаем?
        </a>
    </li> -->
    <!-- End /.Menu-item--> 

    <!-- Start .Menu-item -->
    <li class="Menu-item">
        <a href="/contact.html" title="Контакты" class="Menu-item__link">
        Контакты
        </a>
    </li><!-- End /.Menu-item-->    

</ul><!-- End /.Menu-->   

            <!-- Start .Form -->
<form action="" method="" class="Form">

    <!-- Start .Form__group -->
    <div class="Form__group Form__group--one">
        <input type="email" class="Form__input Form__input--group" id="email" name="email" spellcheck="false" autocapitalize="off" required>
        <label for="email" class="Form__label Form__label--group">Электронная почта</label>
        <span class="Form__send">        
            <input type="submit" value="" class="Form__submit" data-toggle="modal" data-target="#exampleModalCenter">          
        </span>
    </div><!-- End /.Form__group -->    

</form><!-- End /.Form -->



            <!-- Modal -->
<div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-body p-3 h-auto text-center">
            Thanks for subscription
        </div>
      </div>
    </div>
  </div>

        </u><!-- End /.container-->

    </nav><!-- End /.Navbar__nav-->

</footer><!-- End /.Navbar-->
    

<!-- =========================
    Scripts   
============================== --> 

<!-- All Main Scripts -->
<script src="src/assets/js/scripts.min.js"></script>

<!-- Mobile - Burger Right Menu -->
<script src="src/assets/js/burger-menu.min.js"></script>

<!-- Common --> 
<script src="src/assets/js/common.js"></script>

</body>
</html>